# infrastructure/telegram_adapter_gemini.py
...
class TelegramAdapterGemini:
    def __init__(self, token, detectar_intencion_usecase: DetectarIntencionGeminiUseCase,
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "¡Hola! Soy DECSA, tu asistente virtual oficial. Estoy aquí para ayudarte con cualquier problema o consulta sobre nuestros servicios eléctricos. ¿En qué puedo ayudarte hoy? Puedes hacer un reclamo, actualizar tus datos o consultar el estado de un reclamo."
        )
    ...
    async def reset(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.redis_client.delete(f"user:{user_id}:estado")
        await update.message.reply_text(
            "Conversación reiniciada. ¿En qué puedo ayudarte ahora? Puedo asistirte con reclamos, actualizar datos o consultar estados."
        )
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        try:
            ...
            if texto_usuario in ["cancelar", "salir"] and estado["fase"] != "inicio":
                ...
                await update.message.reply_text(
                    "✅ Entendido, he detenido el proceso. ¿En qué puedo ayudarte ahora? Puedo asistirte con reclamos, actualizar datos o consultar estados."
                )
                logging.info("Proceso cancelado por el usuario")
                return
            elif texto_usuario in ["cancelar", "salir"] and estado["fase"] == "inicio":
                await update.message.reply_text(
                    "No hay ningún proceso activo para cancelar. ¿En qué puedo ayudarte hoy? Puedo asistirte con reclamos, actualizar datos o consultar estados."
                )
                return

            if estado["fase"] == "inicio":
                logging.info("Fase inicio: Detectando intención con Gemini")
                ...
                try:
                    ...
                    respuesta = resultado.get("respuesta", "No entendí bien tu mensaje. ¿En qué puedo ayudarte hoy? Puedes decirme si quieres hacer un reclamo, actualizar datos o consultar algo.")
                except (json.JSONDecodeError, TypeError) as e:
                    logging.warning(f"Error al parsear respuesta: {respuesta_cruda}, Error: {str(e)}")
                    intencion = "Conversar"
                    respuesta = "No entendí bien tu mensaje. ¿En qué puedo ayudarte hoy? Puedes decirme si quieres hacer un reclamo, actualizar datos o consultar algo."
                ...
                await update.message.reply_text(respuesta)
                await update.message.reply_text(
                    "¿Necesitas algo más? Puedo ayudarte con un reclamo, actualizar datos o consultar estados."
                )
                self.redis_client.hset(estado_clave, "fase", "inicio")
                self.redis_client.hdel(estado_clave, "descripcion")
    ...
    def run(self):
        logging.info("🚀 Bot de Telegram (Gemini) corriendo...")
