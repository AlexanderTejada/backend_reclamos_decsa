...
class TelegramAdapterDPSK:
        ...
        try:
            ...
            if estado["fase"] == "inicio":
                ...
                if estado.get("accion") == "consultar":
                    ...
                    usuario_db1 = self.actualizar_usecase.usuario_repository.obtener_de_db1(
                    ...
                    elif usuario_db1 and campo in usuario_db1:
                        current_value = usuario_db1[campo] or "No disponible"
                ...
                if re.match(r'^\d+$', texto_usuario):
                    ...
                    if codigo == 200:
                        await update.message.reply_text(
                            ...
                            f"- Descripción: {respuesta.get('DESCRIPCION', 'No disponible')}\n"
                            f"- Estado: {respuesta.get('ESTADO', 'No disponible')}\n"
                            f"- Fecha de Reclamo: {respuesta.get('FECHA_RECLAMO', 'No disponible')}\n"
                            f"- Fecha de Cierre: {respuesta.get('FECHA_CIERRE', 'No disponible')}\n"
                            f"- Cliente: {respuesta.get('cliente', {}).get('nombre', 'No disponible')} (DNI: {respuesta.get('cliente', {}).get('dni', 'No disponible')})\n"
                            f"- Número de Suministro: {respuesta.get('numeroSuministro', 'No disponible')}\n"
                            f"- Número de Medidor: {respuesta.get('medidor', 'No disponible')}\n"
                            f"- Dirección: {respuesta.get('direccion', 'No disponible')}"
                        )
                    else:
    ...
    def format_reclamos(self, reclamo_data=None, is_single=False):
        ...
        if is_single and reclamo_data:
            return (
                f"Detalles del reclamo ID {reclamo_data.get('ID_RECLAMO', 'No disponible')}:\n"
                f"- Descripción: {reclamo_data.get('DESCRIPCION', 'No disponible')}\n"
                f"- Estado: {reclamo_data.get('ESTADO', 'No disponible')}\n"
                f"- Fecha de Reclamo: {reclamo_data.get('FECHA_RECLAMO', 'No disponible')}\n"
                f"- Fecha de Cierre: {reclamo_data.get('FECHA_CIERRE', 'No disponible')}\n"
                f"- Cliente: {reclamo_data.get('cliente', {}).get('nombre', 'No disponible')} (DNI: {reclamo_data.get('cliente', {}).get('dni', 'No disponible')})\n"
                f"- Número de Suministro: {reclamo_data.get('numeroSuministro', 'No disponible')}\n"
                f"- Número de Medidor: {reclamo_data.get('medidor', 'No disponible')}\n"
                f"- Dirección: {reclamo_data.get('direccion', 'No disponible')}"
            )
        else:
            respuesta, codigo = self.consulta_estado_usecase.ejecutar(reclamo_data) if reclamo_data else (None, 404)
